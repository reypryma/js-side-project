...
function generate() {
    if (item.value === "0") {
        ...
        alert("Value exceeds the number of paragraphs available.");
    } else {
        // Clear previous content
        dataContainer.innerHTML = "";

        const shuffleParagraphs = [...paragraphs]; // Clone the array
        shuffle(shuffleParagraphs);

        const selectedParagraphs = shuffleParagraphs.slice(0, item.value);

        // Check if item.value is greater than or equal to 6
        const useColumns = item.value >= 6;
        ...
        if (useColumns) {
            ...
            while(twoColumn < 2){
                ...
                if (twoColumn === 0) {
                    // Populate the first column with the first 5 paragraphs
                    const firstColumnParagraphs = selectedParagraphs.slice(0, 5);
                    firstColumnParagraphs.forEach((paragraph) => {
                        const pElement = document.createElement("p");
                        pElement.innerText = paragraph;
                        column.appendChild(pElement);
                    });
                } else {
                    // Populate the second column with the remaining paragraphs
                    const secondColumnParagraphs = selectedParagraphs.slice(5);
                    secondColumnParagraphs.forEach((paragraph) => {
                        const pElement = document.createElement("p");
                        pElement.innerText = paragraph;
                        column.appendChild(pElement);
                    });
            ...
            selectedParagraphs.forEach((paragraph) => {
                const pElement = document.createElement("p");
                pElement.innerText = paragraph;
                dataContainer.appendChild(pElement);
            });
        }
    }
}